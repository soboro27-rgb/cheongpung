...
async def login(request: Request, db: Session = Depends(get_db)):
    ...
    if user and _verify(password, user.password_hash):
        ...
        request.session["operator_id"]        = user.operator_id
        request.session["operator_name"]      = user.operator.name if user.operator else ""
        request.session["operator_fee_type"]  = user.operator.fee_type.value if user.operator else ""
        request.session["operator_fee_value"] = user.operator.fee_value if user.operator else 0
